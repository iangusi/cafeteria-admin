...
from .forms import (
    ...
    AsistenciaRegistroForm, EmpleadoForm, ClienteForm, ClienteCreateForm,
    VentaForm, VentaItemFormSet
)
from .utils import (
...
def venta_edit(request, pk):
    ...
    if not venta.puede_modificar():
        messages.error(request, 'No se puede editar una venta finalizada.')
        return redirect('ventas_list')
...
def venta_cancel(request, pk):
    venta = get_object_or_404(Venta, pk=pk)
    # solo se permite cancelar (eliminar) si sigue pendiente
    ...
    try:
        ...
        messages.error(request, f'No se pudo eliminar la venta: {e}')
    return redirect('ventas_list')
...
@require_POST
@transaction.atomic
def venta_finalize(request, pk):
    venta = get_object_or_404(Venta, pk=pk)
    if venta.estado != 'pendiente':
        messages.error(request, 'Esta venta ya fue procesada.')
        return redirect('ventas_list')

    # aplicar cambios recibidos en case user edited title/client but didn't click save
    ...
    # aplicar puntos automáticos y finalizar (descontar inventario y asignar puntos)
    ok = venta.finalizar()
    if ok:
        messages.success(request, 'Venta finalizada: stock descontado y puntos asignados.')
    else:
        messages.error(request, 'No se pudo finalizar la venta.')
    return redirect('ventas_list')


# ----------------- CREAR CLIENTE INLINE (AJAX) -----------------
@require_POST
def cliente_create_ajax(request):
    form = ClienteCreateForm(request.POST)
    if form.is_valid():
        cliente = form.save()
        return JsonResponse({'ok': True, 'cliente_id': cliente.pk, 'cliente_nombre': cliente.nombre})
    return JsonResponse({'ok': False, 'errors': form.errors}, status=400)


# ----------------- PERSONALIZAR (duplica producto y devuelve id) -----------------
@require_POST
def venta_personalize_create(request):
    """
    POST params: item_index (or item id), producto_id, venta_id
    Creamos un nuevo Producto duplicado y retornamos su id para que el frontend lo asocie al row.
    """
    producto_id = request.POST.get('producto_id')
    if not producto_id:
        return JsonResponse({'ok': False, 'error': 'producto_id requerido'}, status=400)
    producto = get_object_or_404(Producto, pk=producto_id)
    nuevo = duplicate_producto(producto)  # duplicate utility must copy Producto and ProductoReceta
    return JsonResponse({'ok': True, 'nuevo_producto_id': nuevo.pk, 'nuevo_producto_nombre': nuevo.nombre})